...
# Подтверждение можно передать заранее: RESTORE_YES=1 scripts/restore.sh …
# Нужно для проверок по расписанию, где спросить некого. Значение
# латинское намеренно: кириллица в переменной окружения по дороге через
# ssh искажается и сравнение молча не срабатывает.
case "${RESTORE_YES:-}" in
  1|y|yes|Y|YES) say "подтверждение получено переменной RESTORE_YES" ;;
  *)
